...
- **Preview scheduling is deterministic and modular.** `H3 Last Active VHS Preview Barrier` makes the highest enabled intermediate preview finish before final assembly requests the clip latents. It now uses the same **Input Count → Update inputs** pattern as the final streamers, so AV Extension and Music Video show only the preview sockets they need instead of the full 64-slot backend range. The VHS refresh workaround also reloads completed previews when VideoHelperSuite reuses the same temporary filename.
